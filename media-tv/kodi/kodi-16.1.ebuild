...
LICENSE="GPL-2"
SLOT="0"
IUSE="airplay alsa avahi bluetooth bluray caps cec dbus debug gles java midi mysql nfs +opengl profile pulseaudio +samba sftp +system-ffmpeg test +texturepacker udisks upnp upower +usb vaapi vdpau webserver +X"
...
COMMON_DEPEND="${PYTHON_DEPS}
	...
	media-sound/wavpack
	system-ffmpeg? ( >=media-video/ffmpeg-2.8:=[encode] )
	avahi? ( net-dns/avahi )
	nfs? ( net-fs/libnfs:= )
...
src_unpack() {
	[[ ${PV} == "16.1" ]] && git-r3_src_unpack || default
}

src_prepare() {
	epatch "${FILESDIR}"/${PN}-16.1-no-arm-flags.patch #400617
	epatch "${FILESDIR}"/${PN}-16.1-texturepacker.patch
	...
	if [[ ${PV} == "16.1" ]] || use java ; then #558798
...
src_configure() {
	...
	[[ ${PV} != "16.1" ]] && export ac_cv_path_JAVA_EXE=$(which $(usex java java true))
